...
ZLIB_BUILD_COMMANDS="./configure --prefix=\"$prefix\" --static && make && make install"
...
OPENSSL_BUILD_COMMANDS="./config --prefix=\"$prefix\" --openssldir=\"$prefix/ssl\" no-tests no-dso && make && make install"
...
OPENSSH_BUILD_COMMANDS="autoreconf && ./configure --prefix=\"$prefix\" --exec-prefix=\"$prefix\" --sysconfdir=\"$prefix/etc\" --with-privsep-user=nobody --with-ssl-dir=\"$prefix\" --with-default-path=\"$prefix/bin\" --without-pam --disable-libsystemd && make && make install"
